const { S, src, sandbox, fail, fs, path, noop } = require('./app_js_harness');
const vm = require('vm');

// Round 14 official asset IDs map to the active fantasy seats.
try {
...
// The R17 return keeps Tsunoda in history/held teams and uses the published
// two-zero assumption only for the returning drivers' price forecast.
try {
  const live = {
    round: 17,
    driver_assets: { override_active: true },
  ...
  const official = { rounds: {
    '15': { drivers: { HAD: 30, LAW: 25 } },
    '16': { drivers: { HAD: 20, LAW: 16 } },
  } };
  const summary = { rounds: [{ round: 15, has_actual: true }, { round: 16, has_actual: true }] };
  S.setPriceLoaderState(live, official, summary);
  const tsunoda = S.findDriverAsset('TSU_RACING_BULLS');
  if (!tsunoda?.held_only || tsunoda.expected_points !== 0 || tsunoda.current_price !== 9.7) {
    fail('R17 inactive Tsunoda asset was not preserved as a held-only historical pick');
  }
  if (S.normalizeOfficialAssetId({ asset_type: 'driver', asset_id: '130', name: 'Yuki Tsunoda' }) !== 'TSU_RACING_BULLS') {
    fail('R17 official Tsunoda holding was mapped to an active driver');
  }
  for (const id of ['HAD', 'LAW']) {
    const item = live.drivers.find(driver => driver.driver_id === id);
    const assumed = S.predictPriceChange(item, 12);
    const historicalTotal = id === 'HAD' ? 50 : 41;
    if (!assumed.priceWindowAssumedZero || assumed.pastScores.join(',') !== '0,0'
        || assumed.cumulativeTotal !== historicalTotal) {
      fail(`${id} did not use provisional zeros while retaining recorded history`);
    }
    delete live.price_change_assumption;
    const regular = S.predictPriceChange(item, 12);
    live.price_change_assumption = { round: 17, driver_ids: ['HAD', 'LAW'], previous_two_scores: [0, 0] };
    if (Math.abs((assumed.ptsForGood - regular.ptsForGood) - historicalTotal) > 1e-6) {
      fail(`${id} rise threshold did not reflect two zero weeks`);
    }
  }
} catch (e) {
...
// Team Compare sources keep saved-team financial state separate from the
...
// Pit Wall store/events, finance separation, entitlement state, and the
...
// Static shell contracts: the workspace/menu are present and Beat V13
...
try {
  ...
  if (!/styles\.css\?v=\d+/.test(index) || !/engagement\.css\?v=\d+/.test(index) || !/team-state\.js\?v=\d+/.test(index) || !/final-fix\.js\?v=\d+/.test(index) || !/app\.js\?v=\d+/.test(index) || !/members\.js\?v=\d+/.test(index) || !/engagement\.js\?v=\d+/.test(index)) fail('changed static asset cache versions are missing');
  if (!/team-state\.js\?v=\d+[\s\S]*app\.js\?v=\d+[\s\S]*members\.js\?v=\d+/.test(index)) fail('team-state must load before app and members');
  if (!/final-fix\.js\?v=\d+[\s\S]*app\.js\?v=\d+/.test(index)) fail('Final Fix calculations must load before app');
  ...
  if (!engagement.includes('Sign out of BoxBox') || !engagement.includes('/api/members/sign-out/')) fail('access hub is missing the visible unified sign-out action');
...
// renderSwapRow produces the swap-delta markup.
try {
...
// Transfer Advisor display values follow its selected points basis.
...
// Current-round manual upgrade overlays flow into optimizer bases,
...
// Team Compare scoring includes normal 2x / 3x boost and CI totals.
try {
  const drivers = [
...
// Explicit scoring basis must choose its own boost target even when another
// optimizer mode has changed the app's current basis.
try {
  const scoring = sandbox.BoxBoxOptimizerScoring;
  const drivers = [
    { driver_id: 'A', projected_points: 30, expected_points: 4, mc_total_p5: -5, mc_total_p95: 40 },
    { driver_id: 'B', projected_points: 10, expected_points: 20, mc_total_p5: -2, mc_total_p95: 30 },
  ];
  const constructors = [{ constructor_id: 'C', projected_points: 5, expected_points: 5, mc_total_p5: -3, mc_total_p95: 12 }];
  S.setTransferRenderState('risk_adjusted', { drivers, constructors }, [], []);
  const projected = scoring.scoreTeamPicks(drivers, constructors, 'none', 'projected');
  const risk = scoring.scoreTeamPicks(drivers, constructors, 'none', 'risk_adjusted');
  const triple = scoring.scoreTeamPicks(drivers, constructors, '3x_boost', 'projected');
  const protectedFloor = scoring.scoreTeamPicks(drivers, constructors, 'no_negative', 'projected');
  if (projected.expected !== 75 || projected.floor !== -15 || projected.ceiling !== 122 || projected.boostedDriverId !== 'A') {
    fail('projected team scoring or interval totals changed');
  }
  if (risk.expected !== 49 || risk.boostedDriverId !== 'B') fail('risk-adjusted boost target changed');
  if (triple.expected !== 115 || triple.secondBoostedDriverId !== 'B') fail('3x Boost targets changed');
  if (protectedFloor.floor !== 0) fail('No Negative did not protect the interval floor');
} catch (e) {
  fail('optimizer scoring module threw: ' + e.message);
}

// Final Fix race math counts finish, net positions, overtakes and bonuses.
try {
  ...
  const comparison = S.finalFixCompare({
    outDriver: { current_price: 25, predicted_quali: 4, projected_points_race: 10.9 },
    inDriver: { current_price: 28, predicted_quali: 3, projected_points_race: 14 },
    bank: 4,
    boosted: true,
    outgoingPoints: { total: 10 },
    incomingPoints: { total: 19 },
  });
  if (!comparison.affordable || comparison.bankedQuali !== 7
      || comparison.holdTotal !== 27 || comparison.switchTotal !== 45
      || comparison.scenarioDelta !== 18 || Math.abs(comparison.modelDelta - 6.2) > 1e-9) {
    fail(`Final Fix comparison changed banked points or boost scope: ${JSON.stringify(comparison)}`);
  }
} catch (e) {
  fail('Final Fix scoring helper threw: ' + e.message);
}

// Known grid penalties remain distinct from qualifying results.
try {
...
// Landing-page price history skips full actual files only when official
...
// Final Fix owns its form lifecycle; app.js only mounts it with current data.
try {
  const elements = {};
  const events = {};
  sandbox.document.getElementById = id => elements[id] ||= {
    value: '', innerHTML: '', textContent: '', className: '', checked: false,
    classList: { remove() {} },
    addEventListener(name, fn) { (events[id] ||= {})[name] = fn; },
  };
  sandbox.document.querySelector = () => null;
  sandbox.document.querySelectorAll = () => [];
  S.setPriceLoaderState({ final_fix: { qualifying_locked: true }, drivers: [
    { driver_id: 'ANT', name: 'Andrea Kimi Antonelli', current_price: 22, predicted_quali: 4, predicted_grid: 7, grid_penalty_places: 3, predicted_finish: 5, projected_points_race: 11.2, mc_overtakes_mean: 3 },
    { driver_id: 'NOR', name: 'Lando Norris', current_price: 28, predicted_quali: 2, predicted_grid: 2, predicted_finish: 2, projected_points_race: 19, mc_overtakes_mean: 2 },
  ] }, {});
  vm.runInContext('setupFinalFixTool()', sandbox);
  if (elements.ffOutDriver.value !== 'ANT' || elements.ffInDriver.value !== 'NOR') fail('Final Fix default drivers changed');
  if (!elements.ffOutFacts.innerHTML.includes('3-place grid penalty')) fail('Final Fix lost the penalised race start');
  if (!elements.finalFixStatus.className.includes('ready')) fail('Final Fix lost its qualifying-lock status');
  if (!elements.finalFixResult.innerHTML.includes('7 Qualifying points stay banked')) fail('Final Fix lost banked qualifying points');
  elements.ffInFinish.value = 'dnf';
  elements.ffInOvertakes.value = '3';
  elements.ffBank.value = '1';
  events.ffBank.input();
  if (!elements.finalFixResult.innerHTML.includes('Need $5.0M more')
      || !elements.finalFixResult.innerHTML.includes('DNF')) {
    fail('Final Fix controls did not recalculate the scenario');
  }
} catch (e) {
  fail('Final Fix form integration threw: ' + e.message);
}

console.log('PASS: app.js optimizer, team comparison, and scoring behavior.');
