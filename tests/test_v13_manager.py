...
def test_v13_replay_preserves_full_season_score_and_provenance() -> None:
    ...
    assert replay["base_points_before_final_fix"] == 2537.0
    assert replay["final_fix_adjustment"] == 4.0
    assert replay["total_points"] == 2541.0
    assert replay["final_budget"] == 124.3
...
def test_v13_uses_only_qualifying_locked_data_for_final_fix() -> None:
    ...
    assert final_fix["outgoing"] == "LEC"
    assert final_fix["incoming"] == "NOR"
    assert final_fix["actual_gain"] == 4.0
    assert round_13["actual_points"] == 225.0
    assert round_13["cumulative_points"] == 2541.0

    state = payload["current_state"]
    assert "NOR" in state["drivers"]
    assert "LEC" not in state["drivers"]
...
def test_r14_early_thoughts_are_frozen_from_a_pre_lock_archive() -> None:
    decision = v13._load_json(
        v13.DECISION_DIR / "round14_pre_fp.json"
    )
    ...
    assert decision["drivers"] == ["ANT", "HAM", "HAD", "COL", "BOR"]
    assert decision["constructors"] == ["ferrari", "racing_bulls"]
