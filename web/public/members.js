    ...
    function renderSignedIn() {
        ...
        const cardHtml = teams.map(team => {
            ...
            return `<article class="pit-wall-team-card${selected ? ' selected' : ''}${complete ? ' complete' : ''}" data-team-slot="${team.slot}">
                ...
                <div class="pit-wall-team-tools"><input type="text" maxlength="60" value="${escapeHtml(team.name)}" aria-label="Team ${team.slot} name" data-team-name="${team.slot}"${active ? '' : ' disabled'}><button type="button" class="pit-wall-text-button" data-team-rename="${team.slot}"${active ? '' : ' disabled'}>Rename</button>${f1 ? `<span class="pit-wall-team-link">Official F1 T${escapeHtml(f1.team_slot || team.slot)} connected</span>` : '<span class="pit-wall-team-link">Manual team</span>'}</div>
            </article>`;
        }).join('');
        const selected = storeTeam(selectedSlot);
        const selectedOfficial = selected?.f1_link;
        const officialHtml = selectedOfficial
            ? `<div class="pit-wall-official linked"><div><span>Official-team synchronization · ${teamLabel(selected)}</span><strong>Connected · ${escapeHtml(selectedOfficial.official_team_name || selectedOfficial.name || 'T' + selectedSlot)}</strong><small>${selectedOfficial?.last_synced_at ? `Last synchronized ${friendlyDate(selectedOfficial.last_synced_at)}` : 'Connected and ready to synchronize.'}</small><p>Copies the latest locked F1 Fantasy lineup—normally your final team from the previous race—into Team ${selectedSlot} for this round. It replaces this slot's saved picks and updates its squad value and cash in bank only when you click synchronize.</p></div><div class="pit-wall-actions"><button type="button" class="pit-wall-save" data-team-sync="${selectedSlot}"${active ? '' : ' disabled'}>Synchronize Team ${selectedSlot}</button><button type="button" class="pit-wall-link-danger" data-team-unlink="${selectedSlot}"${active ? '' : ' disabled'}>Disconnect</button></div></div>`
            : `<div class="pit-wall-official"><span>Connect an official team to ${teamLabel(selected)}</span><p>Join the Box Box league first, then search the exact F1 Fantasy team name. Its official T1/T2/T3 slot connects to the matching saved Team slot. Connecting does not replace the lineup you already saved; you choose when to synchronize it.</p><form id="pitWallF1Search"><div class="pit-wall-f1-fields"><label><small>Exact F1 Fantasy team name</small><input name="team_name" type="search" placeholder="e.g. Boxed In" minlength="2" maxlength="100" required></label><button type="submit">Find my team</button></div></form><div class="pit-wall-search-results" id="pitWallF1Results"></div></div>`;
        ...
        ${active ? `<p>Your saved lineup stays exactly as you left it until you edit it or explicitly synchronize its connected official F1 team. Each of the three slots keeps its own picks, spending power, cash, transfers and chips.</p><div class="pit-wall-team-grid">${cardHtml}</div><div class="pit-wall-actions"><button type="button" class="pit-wall-save" id="pitWallSaveTeam">Save Team ${selectedSlot}</button><button type="button" class="pit-wall-secondary" id="pitWallEditSelectedTeam">Edit selected team in Transfer Advisor</button><button type="button" class="pit-wall-secondary" id="pitWallCompareTeams">Compare teams</button><button type="button" class="pit-wall-secondary" id="pitWallSignOut">Sign out</button></div><label class="pit-wall-pref"><input type="checkbox" id="pitWallSimEmails" ${dashboard.profile?.email_simulation_updates !== false ? 'checked' : ''}> Email me personalized simulation updates</label>${officialHtml}` : `<p>Your Ko-fi entitlement is no longer active. Your saved data is retained and returns when you renew.</p><div class="pit-wall-team-grid">${cardHtml}</div><div class="pit-wall-actions"><a class="pit-wall-save" href="https://ko-fi.com/boxboxf1fantasy/tiers" target="_blank" rel="noopener">Renew on Ko-fi</a><button type="button" class="pit-wall-secondary" id="pitWallSignOut">Sign out</button></div>`}
        ...
        panel.querySelector('#pitWallF1Search')?.addEventListener('submit', async event => {
            ...
            try { const query = encodeURIComponent(form.elements.team_name.value.trim()); const data = await request(`/api/members/team/?action=f1-search&q=${query}`); results.innerHTML = data.teams?.length ? data.teams.map(team => `<button type="button" data-team-link-token="${escapeHtml(team.link_token)}" data-official-slot="${Number(team.slot)}"><strong>${escapeHtml(team.name)} · T${team.slot}</strong><small>${escapeHtml(team.manager || '')}${team.rank ? ` · Rank #${Number(team.rank).toLocaleString()}` : ''} · saves to Team ${Number(team.slot)}</small></button>`).join('') : `<span>No match in the latest published league standings. Check the exact team name and that you have joined the Box Box league. Newly joined teams may appear on the official site before this feed updates. You can save your lineup manually in Pit Wall while waiting.</span>`;
                results.querySelectorAll('button').forEach(resultButton => resultButton.addEventListener('click', async () => { resultButton.disabled = true; const slot = Number(resultButton.dataset.officialSlot); try { await store?.linkOfficial(slot, { link_token: resultButton.dataset.teamLinkToken }); status(`Official team connected to Team ${slot}. Choose Synchronize Team ${slot} when you are ready to import it.`, 'success'); renderSignedIn(); } catch (error) { status(error.message, 'error'); resultButton.disabled = false; } }));
            } catch (error) { results.innerHTML = `<span>${escapeHtml(error.message)}</span>`; } finally { button.disabled = false; }
        });
        panel.querySelectorAll('[data-team-sync]').forEach(button => button.addEventListener('click', async () => {
            const slot = Number(button.dataset.teamSync);
            if (!window.confirm(`Synchronize the latest locked official F1 lineup into Team ${slot}? This replaces Team ${slot}'s saved picks and finances.`)) return;
            const originalLabel = button.textContent;
            button.disabled = true;
            button.textContent = 'Synchronizing…';
            try {
                await store?.synchronizeOfficial(slot, window.BoxBoxTeamMemory?.currentRound?.());
                await loadSession(false);
                status(`Team ${slot} now matches your latest locked official F1 lineup and is saved for this round.`, 'success');
            } catch (error) {
                status(error.message, 'error');
            } finally {
                button.disabled = false;
