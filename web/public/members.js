(function () {
    ...
    function renderSignedIn() {
        ...
        const officialHtml = f1Link
            ? `<div class="pit-wall-official linked"><div><span>F1 Fantasy connected</span><strong>${escapeHtml(f1Link.official_team_name)} · T${f1Link.team_slot}</strong><small>${f1Link.last_synced_at ? `League feed refreshed ${friendlyDate(f1Link.last_synced_at)}` : 'Connected · waiting for first refresh'}${f1Snapshot?.round ? ` · Round ${f1Snapshot.round}` : ''}</small><small>This connection works through the Box Box league. Your team must remain in that league; no F1 password or browser cookie is stored.</small></div><div class="pit-wall-actions"><button type="button" class="pit-wall-secondary" id="pitWallSyncF1">Refresh official team</button><button type="button" class="pit-wall-link-danger" id="pitWallDisconnectF1">Disconnect</button></div></div>`
            : `<div class="pit-wall-official"><span>Connect F1 Fantasy</span><p><strong>Required:</strong> your team must be in the Box Box F1 Fantasy league before Pit Wall can find or refresh it. The connection reads that league's official feed; your F1 password and browser cookies are never requested or stored.</p><div class="pit-wall-f1-join"><span>Step 1 · Required league membership</span><strong>Join with code: P1JZAGNMP04</strong><a href="https://fantasy.formula1.com/en/leagues/join/P1JZAGNMP04" target="_blank" rel="noopener">Join the Box Box league on F1 Fantasy ↗</a></div><form id="pitWallF1Search"><div class="pit-wall-f1-fields"><label><small>Step 2 · After joining, enter your exact team name</small><input name="team_name" type="search" placeholder="e.g. Boxed In" minlength="2" maxlength="100" required></label><button type="submit">Find my team</button></div></form><p class="pit-wall-f1-help">If you have just joined, wait a moment for F1's league feed to update. Then choose the correct T1/T2/T3 slot. You do not need to enter a rank.</p><div class="pit-wall-search-results" id="pitWallF1Results"></div></div>`;
        ...
        panel.querySelector('#pitWallF1Search')?.addEventListener('submit', async event => {
            ...
            try {
                ...
                results.innerHTML = data.teams.length ? data.teams.map(team => `<button type="button" data-team-link-token="${escapeHtml(team.link_token)}"><strong>${escapeHtml(team.name)} · T${team.slot}</strong><small>${escapeHtml(team.manager || '')}${team.rank ? ` · Rank #${Number(team.rank).toLocaleString()}` : ''}</small></button>`).join('') : `<span>No exact match is visible yet. Join the Box Box league with code <strong>${escapeHtml(data.join_code || 'P1JZAGNMP04')}</strong>, then return here and search again. <a href="${escapeHtml(data.join_url)}" target="_blank" rel="noopener">Join the league ↗</a></span>`;
