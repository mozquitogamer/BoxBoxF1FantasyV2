...
- **Grid-anchoring (`grid_anchor_weight()` in track_classifications.py):** on hard-to-overtake circuits the predicted race finish is blended toward the grid, scaled by `overtaking_difficulty` — 0 at/below difficulty 6 (normal tracks untouched), → 0.85 at Monaco. Only 5/22 rounds affected. NB: hard-track magnitudes (FP weight 0.80, anchor 0.85) are domain-knowledge defaults — no completed Monaco-tier 2026 round to calibrate against yet.
