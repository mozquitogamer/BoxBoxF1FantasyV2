...
- **Layer 2 — FP Telemetry (40+ features):** Lap times, sectors, degradation, long-run pace, compound-specific features (soft/medium/hard), relative pace deltas. Long runs use the shared `pipeline/fp_long_runs.py` extractor: each physical run is isolated by `(session, stint_number, compound)`, all race compounds (including SOFT) qualify, and a candidate needs at least 5 raw laps with at least 4 clean laps after tyre-life-aware in/out/cool-lap and traffic-spike trimming. The headline is lap-weighted within one comparable session only (FP2 first, then FP1, then FP3); missing evidence stays NaN rather than being labelled slow. Backfilled across 2022-2026 via `pipeline/backfill_fp_history.py` (raw FastF1 FP for those years was on disk but `02`/`03` only ever extracted the current season), so **57.4% of training rows now carry FP** (2022:65% → 2025:94%; 2020-2021 have no raw FP). Was 2026-only (~3%) until 2026-06-15 — that backfill let the model weight FP pace natively (validated quali walk-forward MAE −0.13, better in all 5 years; `pace_rank` rose into the quali top-8). The inference-time FP-pace blend (see ML Models) runs on top and stays additive.
...
- **Deadline reality:** the F1 Fantasy team-lock is BEFORE qualifying, so `post_fp` is the only actionable prediction (`post_quali` is retrospective). The two adjustments below exist because under-using FP telemetry hurts the one prediction that matters. Both run in `06_run_predictions.py` and OVERWRITE the raw scores (`predicted_quali_raw`/`predicted_race_raw`) so they flow to the deterministic finish AND the MC (which re-ranks from those raw scores).
- **FP-pace quali blend (`FP_QUALI_BLEND_TUNABLES` in 06):** after the quali model runs, its scores are blended (z-score space) toward this weekend's FP pace — a COMPOSITE of best lap + best-3 + best-5 lap averages (rewards repeatable pace over a one-off banker; backtests as a better quali predictor than the model alone, MAE 2.24 vs 2.36/2.88). Weight is track-scaled via `fp_quali_blend_weight()`: 0.6 normal → 0.80 at Monaco-level (one-lap tracks). Self-skips when no FP pace exists (pre_fp/horizon stay priors-only). Race finish is NOT FP-blended (FP long-runs predict finishes poorly).
- **Phase-aware grid-anchoring (`grid_anchor_weight()` in track_classifications.py):** anchoring stays OFF for `pre_fp` predictions because the grid is still a priors-only model estimate already consumed by the race model. Once at least 10 drivers have usable current-weekend FP quali-pace evidence, the predicted race finish is blended toward that evidence-backed grid, scaled by `overtaking_difficulty` — 0 at/below difficulty 6 (normal tracks untouched), → 0.85 at Monaco. Post-quali archives may also anchor to the actual grid, though they are retrospective and not actionable for Fantasy. Only 5/22 rounds are eligible. NB: hard-track magnitudes (FP weight 0.80, anchor 0.85) are domain-knowledge defaults — no completed Monaco-tier 2026 round to calibrate against yet.
...
**Post-qualifying lock:** when `predictions.parquet` contains a complete `actual_quali_position` column, the MC freezes both the official qualifying classification and `predicted_grid_position` (the confirmed penalised starting grid) across all 10K iterations. Only the race is simulated. `simulation_params.qualifying_locked=true` records this state, and the export includes race-only P5/P25/P50/P75/P95 fields (`mc_race_pts_*`) for diagnostics. Qualifying points come from the official classification; grid penalties affect only the race start and positions gained/lost.
...
- Position noise multiplier (1.0× NONE → 1.15× HIGH rain; MEDIUM = 1.08×)
- DNF rate multiplier (1.0× → 1.25×; MEDIUM = 1.13×) capped at 0.60 absolute probability
...
The rain curve was recalibrated on 2026-07-18 against the observed ~1.23× full-wet DNF uplift, removing the former MEDIUM-risk cliff while retaining wider intervals and small wet-skill shifts. Sidecar metadata in `monte_carlo_fantasy.json::simulation_params::weather_adjustments_active` is surfaced to the frontend so driver cards show `🌧 Wet` / `🥶 Cool` badges and the weather widget explains what's being adjusted.
...
- **Lineup optimizer** — five modes: Build Fresh Lineup, Transfer Advisor, Multi-Week Planner, Final Fix, and Team Compare. The fresh-lineup search brute-forces C(22,5)xC(11,2)=1.4M combinations with 4 strategies (Max Points, Max Value, Budget Builder, Balanced). Lock/exclude picks use left-click/right-click.
- **5 general optimizer chips:** Limitless (no budget cap), 3x Boost (best driver 3x + second-best 2x), Wild Card (unlimited transfers), No Negative (negatives become 0), and Autopilot (auto 2x on best).
- **Final Fix calculator** — a dedicated post-qualifying mode, not a generic optimizer/planner chip. It compares holding one driver with replacing them for the Grand Prix. Both choices retain the outgoing driver's banked official qualifying points. Race scoring uses the confirmed penalised grid, selected finish, positions gained/lost, editable overtakes, and optional FL/DOTD. A transferred 2x Boost multiplies only the race portion. The model comparison is permanently fixed to deterministic **Projected** race points (`projected_points_race`); it never reads the Balanced/Risk-adjusted selectors or module-level `optimizeBasis`.
- **Transfer Advisor** — given current team + budget + free transfers + max extra transfers (0-2), finds optimal swaps respecting locked/excluded picks. Branch-and-bound search over a driver candidate pool = union of (top-N by strategy score) ∪ (top-M by PPM) ∪ (K cheapest) ∪ current drivers — the PPM/cheapest sub-pools surface budget-enabler picks a pure score pool would miss. Per-swap shows net £ cost + points delta; each option shows net gain vs holding + gain-per-transfer. Filters out transfers worse than keeping current team. All constants live in the `TA_TUNABLES` block at the top of `app.js`.
...
- **Post-quali scoring keeps two position concepts:** `actual_quali_position` is the official qualifying classification used for banked qualifying points; `predicted_grid_position` is the confirmed penalised race start used for positions gained/lost. Never replace one with the other. In post-quali MC both are fixed, and Final Fix retains the outgoing driver's qualifying points while swapping only deterministic `projected_points_race`.
...
- **FP stint-semantics guard:** `03_extract_features.py` and `backfill_fp_history.py` stamp feature parquets with `fp_stint_semantics_version`; `05_train_models.py` stamps the trained feature manifest; `04_build_model_inputs.py` and `06_run_predictions.py` hard-fail on missing/mismatched versions. This prevents an interrupted rebuild from silently combining legacy cross-session stint features with corrected models (or corrected features with legacy models).
...
- **SEO freshness dates must stay truthful:** `pipeline/14_build_seo_pages.py::SEO_CONTENT_LASTMOD` is bumped only for significant generated-template, metadata, structured-data or internal-link changes. Normal pipeline runs use prediction/article/changelog/video source dates and must not stamp every sitemap/feed URL with the run date.
...
## Pit Wall member workspace

The paid Pit Wall member contract is documented operationally in [`docs/KOFI_PIT_WALL_ACTIVATION.md`](docs/KOFI_PIT_WALL_ACTIVATION.md). The current private workspace is a three-slot model backed by `infrastructure/supabase/005_pit_wall_three_team_workspace.sql`, applied after migrations `001` through `004`.

- Each member may retain Team 1, Team 2, and Team 3 with independent names, lineups, source, finances, chip state, and season/round history. Existing default teams migrate to stable Team 1; the first saved team becomes primary only when no primary exists.
- `squad_value_millions`, `bank_millions`, and `spending_power_millions` are distinct. Legacy `budget_millions` remains an alias for spending power. Never infer squad value or bank from a legacy budget. Official F1 `teambal` is bank only.
- History is idempotent by team/season/round. Chips are season-aware for `limitless`, `3x_boost`, `wild_card`, `no_negative`, `autopilot`, and `final_fix`, with `unknown`, `available`, or `used` status and optional `used_round`.
- Official F1 links and snapshots are slot-specific. Sync is non-destructive; only an explicit **Apply official** action may replace a saved manual lineup. Personalized notifications use the primary team only.
- Inactive members may read retained teams, snapshots, history, and chip state, but saves, renames, primary changes, official refreshes, and recommendations remain entitlement-gated.

When changing this contract, preserve the legacy singular `team`/`budget_millions` response aliases until all clients have migrated. Verify with `node web/tests/member-team-contract.test.js` and `node web/tests/f1-fantasy.test.js`, plus a signed-in mobile smoke test for slot saves, primary selection, official sync/apply, and inactive read-only access.
