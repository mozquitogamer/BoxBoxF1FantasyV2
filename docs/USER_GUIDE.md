...
- **Long-run pace uses real, comparable race runs.** FP1, FP2 and FP3 stint numbers reset, so the pipeline keeps every session, stint and tyre compound separate instead of accidentally joining unrelated laps. Soft, medium, hard and wet-weather runs qualify with at least five valid laps and four remaining after cleaning; in/out laps, cool-downs, traffic and lock-up spikes are removed. Short soft-tyre qualifying runs do not qualify. On a normal weekend the headline prefers FP2, then FP1, then FP3, and never averages different sessions together. The same runs feed fuel-corrected pace and model features.
