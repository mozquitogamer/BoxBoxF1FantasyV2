...
- **The Analysis tab shows sustained practice runs on every tyre.** Choose a practice session to see each clean or partial run separately, whether it used soft, medium or hard tyres. The table uses the median selected lap and shows gaps only among runs on the same tyre; fuel loads are unknown. Run length and lap pattern determine the evidence label, while tyre compound supplies context. Alternating push/cool laps stay visible in lap detail. The qualifying indicator likewise accepts laps on any compound. The separate, versioned model features retain their existing long-run definition until a full historical retrain validates a change.
