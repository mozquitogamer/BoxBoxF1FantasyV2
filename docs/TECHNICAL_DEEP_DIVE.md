...
- **Compound-specific:** soft_best_lap and soft_avg_lap retain one-lap soft pace; soft_long_run_avg, medium_long_run_avg, hard_long_run_avg and the corresponding soft_degradation, medium_degradation, hard_degradation describe clean race runs on each slick compound. Soft tyres use the same minimum-lap and cleaning rules as medium and hard tyres.
...
The versioned model features are built in `pipeline/03_extract_features.py` with `pipeline/fp_long_runs.py`. Keeping session, stint and compound in the grouping prevents FastF1's per-session stint-number reset from fabricating a weekend-long run. The Analysis tab uses `pipeline/fp_simulations.py` for separate reader-facing evidence: laps remain in chronological order, alternating push/cool patterns are excluded from the run table, and every sustained run appears within its practice session regardless of tyre. The table shows each run's compound and median, with gaps only against runs on that same compound. Compound never determines whether a stint qualifies as a run. This Analysis change does not alter trained model features or archived predictions. Model features are extracted **only** from `all_laps_fp*.parquet`. Sprint qualifying laps are saved separately (`all_laps_sprint_qualifying.parquet`); they can appear in the Analysis tab's pre-qualifying lap table but do not feed model training.
...
**2. Phase-aware grid-anchoring on hard-to-overtake circuits.** The race model ranks on pace, which can predict big grid reshuffles where passing is near-impossible. The anchor stays **off before practice** because a priors-only predicted grid is already a race-model input; anchoring toward it again would double-count an uncertain estimate. Once at least 10 drivers have usable current-weekend FP quali pace, `grid_anchor_weight(circuit_id)` blends the predicted race finish toward that evidence-backed grid, scaled by `overtaking_difficulty`: **0 at/below difficulty 6** (normal tracks untouched, byte-for-byte) ramping to **0.85 at Monaco** (Hungary ≈0.64, Zandvoort/Singapore ≈0.42, Barcelona ≈0.21). Post-quali archives may anchor to the actual grid but are retrospective. Only 5 of 22 rounds are eligible. Pace still decides the finish wherever it physically can be expressed.
